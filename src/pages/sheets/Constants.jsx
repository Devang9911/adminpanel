...
export function ChangeBadge({ type }) {
  ...
  return (
    <span
      className={`inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded-md border whitespace-nowrap ${cfg.bg} ${cfg.text} ${cfg.border}`}
    >
      {cfg.label}
    </span>
  );
...
export function VersionBadge({ type }) {
  ...
  return (
    <span
      className={`text-xs font-medium px-2.5 py-0.5 rounded-full border ${cfg.bg} ${cfg.color} ${cfg.border}`}
    >
      {cfg.label}
    </span>
  );
